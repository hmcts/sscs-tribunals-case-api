...
test.describe.serial('Work Allocation - CTSC - Review Admin action task tests', async () => {
...
test("Review Admin Action task is cancelled automatically when case is void", async ({ ctscReviewAdminActionSteps }) => {
    test.slow();
    caseId = await createCaseBasedOnCaseType('PIP');
    await ctscReviewAdminActionSteps.createReviewAdminActionTask(caseId);
