...
test.describe.skip('Associate case tests', () => {
  /* TODO: Below test needs to be revisited when the defect SSCSSI-860 is fixed.
    ...
    */
  test('As a caseworker associate a case to another case', async ({
    associateCaseSteps
  }) => {
    await associateCaseSteps.associateCaseSuccessfully();
  });

  test('As a caseworker I should not be able associate a case to non-existent case', async ({
    associateCaseSteps
  }) => {
  ...
  /* TODO: Below test needs to be revisited when the relevant bug SSCSCI-877 is fixed.
    ...
    */
  test.describe.fixme(() => {
    test('As a caseworker I should not be able self associate a case', async ({
      associateCaseSteps
    }) => {
