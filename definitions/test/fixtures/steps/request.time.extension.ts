...
export class RequestTimeExtension extends BaseStep {
  ...
  async performAndVerifyRequestTimeExtension(caseId: string) {
    ...
    await this.appealDetailsTab.verifyAppealDetailsPageContentByKeyValue(
      reqTimeExtData.stateField,
      reqTimeExtData.stateValue
    );
    await this.appealDetailsTab.verifyAppealDetailsPageContentByKeyValue(
      reqTimeExtData.reasonField,
      reqTimeExtData.reasonValue
    );
  }

  async verifyTcwWithoutCaseAllocatorRoleCanViewReviewRequestTimeExtensionTask(
    caseId: string
  ) {
  ...
  async verifyTcwWithCaseAllocatorRoleCanViewAndAssignRequestTimeExtensionTask(
    caseId: string
  ) {
  ...
  async verifyTcwAsAnAssignedUserForReviewFtaTimeExtensionRequestTaskCanViewAndCompleteTheTask(
    caseId: string
  ) {
  ...
  async verifyRequestTimeExtensionTaskIsCancelledAutomaticallyWhenTheCaseIsVoid(
    caseId: string
  ) {
