...
export class CreateBundle extends BaseStep {
  ...
  async performUploadBundleResponse(caseId) {
    ...
    await this.bundlesTab.verifyBundlesTabContentByKeyValueForASpan(
      `${bundleTestData.amendBundle}`,
      `${bundleTestData.amendBundleDefaultVal}`
    );
  ...
  async triggerBundleForConfidentialCase() {
    ...
    //Edited bundle
    await this.bundlesTab.verifyEditedBundlesTabContentByKeyValueForASpan(
      `${bundleTestData.stitchStatusLabel}`,
      `${bundleTestData.stitchStatusDone}`,
      1
    );
