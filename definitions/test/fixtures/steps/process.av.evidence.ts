...
export class ProcessAVEvidence extends BaseStep {
  ...
  async acceptEvidenceUploadedByDWP(caseId: string) {
    ...
    await this.avTab.verifyPageContentByKeyValue(
      avEvidenceTestdata.typeField,
      avEvidenceTestdata.typeValue
    );
    await this.avTab.verifyPageContentByKeyValue(
      avEvidenceTestdata.docField,
      uploadResponseTestdata.testaudiofile
    );
    await this.avTab.verifyPageContentByKeyValue(
      avEvidenceTestdata.partyField,
      avEvidenceTestdata.ftaValue
    );
    ...
    await this.documentsTab.verifyPageContentByKeyValue(
      ...
      avEvidenceTestdata.docTypeValue
    );
  ...
  async excludeEvidenceUploadedByDWP(caseId: string) {
    ...
    await this.avTab.verifyPageContentByKeyValue(
      'Audio/video document url',
      uploadResponseTestdata.testaudiofile
    );
    await this.avTab.verifyPageContentByKeyValue(
      'Audio/video party uploaded',
      'FTA'
    );
    await this.avTab.verifydueDates('Date added');
    ...
    await this.documentsTab.verifyPageContentByKeyValue(
      'Type',
      'Audio/Video evidence direction notice'
    );
    await this.documentsTab.verifyPageContentByKeyValue('Bundle addition', 'A');
    await this.documentsTab.verifydueDates('Date added');
  ...
  async verifyTcwWithoutCaseAllocatorRoleCanViewProcessAudioVideoEvidenceTask(
    caseId: string
  ) {
  ...
  async verifyTcwWithCaseAllocatorRoleCanViewAndAssignProcessAudioVideoEvidenceTask(
    caseId: string
  ) {
    ...
    await this.loginUserWithCaseId(
      credentials.amSeniorTribunalCaseWorkerWithCaseAllocatorRole,
      true,
      caseId
    );
    await this.homePage.navigateToTab('Tasks');
    ...
    await this.tasksTab.assignTaskToTcwUser(
      task.name,
      credentials.amTribunalCaseWorker.email
    );
  }

  async verifyTcwCanMarkProcessAudioVideoEvidenceTaskCanViewAndCompleteTheTaskMarkAsDone(
    caseId: string
  ) {
  ...
  async verifyTcwAsAnAssignedUserForProcessAudioVideoEvidenceTaskCanViewAndCompleteTheTask(
    caseId: string
  ) {
  ...
  async verifyProcessAudioVideoEvidenceTaskIsCancelledAutomaticallyWhenTheCaseIsVoid(
    caseId: string
  ) {
    ...
    await this.tasksTab.verifyManageOptions(
      task.name,
      task.assignedManageOptionsForCaseAllocator
    );
