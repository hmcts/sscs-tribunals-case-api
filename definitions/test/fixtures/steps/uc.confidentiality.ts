...
export class UcConfidentiality extends BaseStep {
  ...
  async confirmConfidentialitySetSuccessfully() {
    await this.homePage.navigateToTab('Confidentiality');
    await this.confidentialityTab.verifyConfidentialityRows([
      {
        party: 'Appellant',
    ...
    await this.confidentialityTab.verifyConfidentialityFlagVisible();
  }

  async submitConfidentialityConfirmedEvent() {
    ...
    await this.homePage.navigateToTab('History');
    await this.historyTab.verifyHistoryPageEventLink('Confidentiality Confirmed');
    await this.historyTab.verifyPageContentByKeyValue(
      'Event',
