...
export class Postponement extends BaseStep {
  ...
  async postponeAListAssistCaseWithAPostponement(
    grantRequest: string = 'Grant Postponement'
  ) {
    ...
    await this.postponementPage.inputPostponementActionRequestPageData(
      grantRequest
    );
    await this.postponementPage.submitBtn();
