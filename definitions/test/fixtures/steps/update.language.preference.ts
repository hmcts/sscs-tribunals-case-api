...
export class UpdateLanguagePreference extends BaseStep {
  ...
  async performUpdateLanguagePreference(
    caseId: string,
    ...
    await this.updateLanguagePreferencePage.chooseWelshLanguage(
    ...
    await this.historyTab.verifyEventCompleted(
      languagePreferenceData.captionText
    );

    await this.homePage.navigateToTab('Welsh');
    await this.welshTab.verifyPageContentByKeyValue(
      languagePreferenceData.languagePreferenceQuestionLabelWithinWelshTab,
      languagePreferenceData.welshlanguagePreferenceYesLabel
    );
  }
}
