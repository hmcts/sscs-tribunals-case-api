...
export class ProvideAppointeeDetails extends BaseStep {
  ...
  async performAttemptAppointeeDetailsNonDormantCase(caseId: string) {
    ...
    await this.provideAppointeeDetailsPage.verifyDropDownElementNotVisble(
  ...
  async performNoAppointeeDetails(caseId: string) {
    ...
    await this.provideAppointeeDetailsPage.verifyPageContent(
      appointeeDetailsData.eventName
    );
    await this.provideAppointeeDetailsPage.chooseAssistOption(
      appointeeDetailsData.appointeeDetailsNoLabel
    );
  ...
  async performProvideAppointeeDetails(caseId: string) {
    ...
    await this.provideAppointeeDetailsPage.chooseAssistOption(
      appointeeDetailsData.appointeeDetailsYesLabel
    );
    await this.provideAppointeeDetailsPage.verifyAndPopulateAppointeeDetailsPage(
    ...
    await this.appealDetailsTab.verifyAppealDetailsAppointeeDetails(
      appointeeDetailsData
    );
