...
export class AssociateCase extends BaseStep {
  ...
  async associateCaseSuccessfully() {
    ...
    let hyphenatedSecondCaseId =
      StringUtilsComponent.formatClaimReferenceToAUIDisplayFormat(
        secondPipCaseId
      ).replace(/\s/g, '-');
    ...
    await this.summaryTab.verifyPageContentLinkTextByKeyValue(
      'Related appeal(s)',
      hyphenatedSecondCaseId
    );
  ...
  async associateNonExistentCase() {
    ...
    await this.associateCasePage.associateCase(
      associateCaseTestData.associateCaseNonExistentCase
    );
    await this.associateCasePage.verifyInputErrorMessage(
      associateCaseTestData.associateCaseNonExistentCase
    );
