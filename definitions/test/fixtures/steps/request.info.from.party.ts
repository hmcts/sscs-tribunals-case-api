...
export class RequestInfoFromParty extends BaseStep {
  ...
  async completeRequestInfoFromParty() {
    ...
    await this.requestInfoFromPartyPage.selectPartyToRequestInfoFrom(
      requestInfo.requestInfoPartySelectionDropdownValue
    );
    await this.page.getByRole('button', { name: 'Add new' }).click();

    await this.requestInfoFromPartyPage.inputRequestDetails(
      requestInfo.requestInfoDetailsToRequestInput
    );
    ...
    await expect(
      this.page.locator('form.check-your-answers h2.heading-h2')
    ).toHaveText('Check your answers');
  ...
  async verifyCtscAdminWithoutCaseAllocatorRoleCanViewReviewInformationRequestedTask(
    caseId: string
  ) {
  ...
  async verifyCtscAdminWithCaseAllocatorRoleCanViewAndAssignReviewInformationRequestedTask(
    caseId: string
  ) {
    ...
    await this.tasksTab.assignTaskToCtscUser(
      task.name,
      credentials.amCaseWorker.email
    );
  }

  async verifyCtscAdminAsAnAssignedUserForReviewInformationRequestedTaskCanViewAndCompleteTheTask(
    caseId: string
  ) {
  ...
  async verifyReviewInformationRequestedTaskIsCancelledAutomaticallyWhenTheCaseIsVoid(
    caseId: string
  ) {
