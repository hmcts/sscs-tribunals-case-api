...
export class SupplementaryResponse extends BaseStep {
  ...
  async performSupplementaryResponse(caseId: string) {
    ...
    await this.supplementaryResponsePage.selectFtaState(
      supplementaryResponseData.ftaState
    );
    await this.supplementaryResponsePage.chooseFtaRecommendPoToAttend(
      supplementaryResponseData.poAttendOption
    );
  ...
  async verifyActionUnprocessedCorrespondenceTaskCanBeCancelledManuallyByAllocatedCtscAdmin(
    caseId: string
  ) {
