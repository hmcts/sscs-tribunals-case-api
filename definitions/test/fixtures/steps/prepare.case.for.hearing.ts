...
export class PrepareCaseForHearing extends BaseStep {
  ...
  async verifyRegionalCenterAdminCanViewTheAllocateCaseRolesAndCreateBundleTask(
    caseId: string
  ) {
    // Regional Center Admin views the Allocate Case Roles and Create Bundle - RPC task
    await this.loginUserWithCaseId(
      credentials.amRegionalCenterAdmin,
      false,
      caseId
    );
    ...
    await this.tasksTab.verifyManageOptions(
      task.name,
      task.unassignedManageOptionsForRegionalCenterAdmin
    );
    ...
    await this.tasksTab.verifyPageContentByKeyValue(
      task.name,
      'Assigned to',
      task.assignedToRegionalCenterAdmin
    );
    await this.tasksTab.verifyManageOptions(
      task.name,
      task.assignedManageOptionsForRegionalCenterAdmin
    );
  ...
  async verifyRegionalCenterAdminCanViewAndCompleteTheAllocateCaseRolesAndCreateBundleTask(
    caseId: string
  ) {
    await this.verifyRegionalCenterAdminCanViewTheAllocateCaseRolesAndCreateBundleTask(
      caseId
    );
  ...
  async verifyRegionalCenterTeamLeaderCanViewAndCompleteTheAllocateCaseRolesAndCreateBundleTask(
    caseId: string
  ) {
    // Regional Center Team Leader views the Allocate Case Roles and Create Bundle - RPC task
    await this.loginUserWithCaseId(
      credentials.amRegionalCenterTeamLeader,
      false,
      caseId
    );
    await this.homePage.navigateToTab('Tasks');
    ...
    await this.tasksTab.verifyManageOptions(
      task.name,
      task.unassignedManageOptionsForRegionalCenterTeamLeader
    );
    ...
    await this.tasksTab.verifyPageContentByKeyValue(
      task.name,
      'Assigned to',
      task.assignedToRegionalCenterTeamLeader
    );
    await this.tasksTab.verifyManageOptions(
      task.name,
      task.assignedManageOptionsForRegionalCenterTeamLeader
    );
    await this.tasksTab.verifyNextStepsOptions(
      task.name,
      task.nextStepsOptions
  ...
  async verifyAllocateCaseRolesAndCreateBundleIsCancelledAutomaticallyWhenTheCaseIsStruckOut(
    caseId: string
  ) {
