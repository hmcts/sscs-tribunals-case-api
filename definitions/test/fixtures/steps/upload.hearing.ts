...
export class UploadHearing extends BaseStep {
  ...
  async requestAndGrantAnHearingRecording(caseId: string) {
    ...
    await this.hearingRecordingsTab.verifyPageContentByKeyValue(
      hearingRecordingTestData.hearingTypeKey,
      hearingRecordingTestData.hearingTypeValue
    );
    await this.hearingRecordingsTab.verifyPageContentByKeyValue(
      hearingRecordingTestData.hearingIDKey,
      hearingRecordingTestData.hearingIDValue
    );
    await this.hearingRecordingsTab.verifyPageContentByKeyValue(
      hearingRecordingTestData.hearingVenueKey,
    ...
    await this.documentsTab.verifyPageContentByKeyValue(
      hearingRecordingTestData.requestPartyKey,
      hearingRecordingTestData.requestPartyValue
    );
  ...
  async requestAndRefuseAnHearingRecording(caseId: string) {
    ...
    await this.documentsTab.verifyTitleNotPresent(
      'Released hearing recordings 1'
    );
    await this.documentsTab.verifyPageContentNotPresentByKeyValue(
      hearingRecordingTestData.requestPartyKey,
      hearingRecordingTestData.requestPartyValue
