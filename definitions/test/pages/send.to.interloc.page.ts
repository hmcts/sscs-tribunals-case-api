...
export class SendToInterlocPage {
  ...
  async verifyPageContent() {
    await webAction.verifyPageLabel(
      'h1.govuk-heading-l',
      sendToInterlocData.sendToInterlocHeading
    ); //Heading Text
    await webAction.verifyPageLabel(
      "label[for='prePostHearing']",
      sendToInterlocData.sendToInterlocPrePostHearingFieldLabel
    ); //Field Label
    await webAction.verifyPageLabel(
      "label[for='selectWhoReviewsCase']",
      sendToInterlocData.sendToInterlocCaseReviewFieldLabel
    );
    await webAction.verifyPageLabel(
      "label[for='interlocReferralReason']",
