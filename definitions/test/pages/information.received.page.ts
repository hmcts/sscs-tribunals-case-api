...
export class InformationReceivedPage {
  ...
  async verifyPageContent() {
    await webAction.verifyPageLabel(
      ...
      informationReceivedData.informationReceivedCaption
    ); //Captor Text
    await webAction.verifyPageLabel(
      'h1.govuk-heading-l',
      informationReceivedData.informationReceivedHeading
    ); //Heading Text
    await webAction.verifyPageLabel(
      'label[for="interlocReviewState"]',
      informationReceivedData.informationReceivedFieldLabel
    ); //Field Label
  }
