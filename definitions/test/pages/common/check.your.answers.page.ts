...
export class CheckYourAnswersPage {
  ...
  async verifyCYAPageContentWithPHE(
    headingValue: string,
    benefitCode?: string,
    issueCode?: string
  ) {
  ...
  async verifyCYAPageContentWithUCB(
    headingValue: string,
    benefitCode?: string,
    issueCode?: string
  ) {
  ...
  async verifyPHMEErrorMsg(): Promise<void> {
    ...
    await webActions.verifyTextVisibility(
      'You must upload an edited FTA response document'
    );
    await webActions.verifyTextVisibility(
      'You must upload an edited FTA evidence bundle'
    );
  }
  ...
  async verifyIssueCodeErrorMsg(): Promise<void> {
    await webActions.verifyElementVisibility('#errors');
    await webActions.verifyTextVisibility(
      'Issue code cannot be set to the default value of DD'
    );
  }
}
