...
export class AssociateCasePage {
  ...
  async verifyPageContent() {
    await webAction.verifyPageLabel(
      '.govuk-caption-l',
      associateCaseTestData.associateCaseCaption
    ); //Captor Text
    await webAction.verifyPageLabel(
      '.govuk-heading-l',
      associateCaseTestData.associateCaseHeading
    ); //Heading Text
    await webAction.verifyPageLabel(
      '#linkedCasesBoolean legend span',
      associateCaseTestData.associateCaseRelatedAppealsLabel
    ); //Field Label
    await webAction.verifyPageLabel(
      'label[for="linkedCasesBoolean_Yes"]',
      associateCaseTestData.associateCaseRelatedAppealsYesLabel
    );
    await webAction.verifyPageLabel(
      'label[for="linkedCasesBoolean_No"]',
      associateCaseTestData.associateCaseRelatedAppealsNoLabel
    );
  }
  ...
  async verifyInputErrorMessage(caseNumber: string) {
    let errorMessageText = (
      await this.page.locator('div.form-group span.error-message').textContent()
    ).trim();
    expect(errorMessageText).toEqual(
      `${caseNumber} does not correspond to an existing CCD case.`
    );
  }
