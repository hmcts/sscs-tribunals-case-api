...
export class UploadDocumentFurtherEvidencePage {
  ...
  async verifyPageContent() {
    await webActions.verifyPageLabel(
      'h1.govuk-heading-l',
      uploadDocumentFeData.pageHeading
    ); //Page heading
    await webActions.verifyPageLabel(
      'div#draftSscsFurtherEvidenceDocument h2.heading-h2',
      uploadDocumentFeData.sectionHeading
    ); //Section heading
  }
  ...
  async selectDocumenType(documentType: string) {
    await expect(
      this.page.locator(
        'select#draftSscsFurtherEvidenceDocument_0_documentType'
      )
    ).toBeVisible();
  ...
  async inputFilename(fileName: string) {
    await webActions.inputField(
      'input#draftSscsFurtherEvidenceDocument_0_documentFileName',
      fileName
    );
  }
