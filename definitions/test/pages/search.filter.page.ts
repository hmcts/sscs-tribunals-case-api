...
export class SearchFilterPage {
  ...
  async performSearch() {
    await this.page
      .locator('#wb-jurisdiction')
      .selectOption({ label: 'Tribunals' });
    await this.page
      .locator('#wb-case-type')
      .selectOption({ label: 'SSCS Case 6.4.4-E2E AAT' });
    await this.page
      .locator('#wb-case-state')
      .selectOption({ label: 'Appeal Created' });
    ...
    await expect(
      this.page.locator('#search-result-heading__text')
    ).toContainText('Your cases');
    await expect(
      this.page.locator('#search-result-summary__text')
    ).toContainText('results');
  }

  async validateSearchResults(caseId: number) {
    const locator: Locator = this.page.locator(
      'ccd-search-result:nth-child(1) > table:nth-child(1) > tbody:nth-child(3) > tr'
    );
