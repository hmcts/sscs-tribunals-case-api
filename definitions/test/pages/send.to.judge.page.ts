...
export class SendToJudgePage {
  ...
  async verifyPageContent() {
    await webAction.verifyPageLabel(
      'h1.govuk-heading-l',
      sendToJudgeData.sendToJudgeHeading
    ); //Heading Text
    await webAction.verifyPageLabel(
      "label[for='prePostHearing']",
      sendToJudgeData.sendToJudgePrePostHearingFieldLabel
    ); //Field Label
    await webAction.verifyPageLabel(
      "label[for='tempNoteDetail']",
      sendToJudgeData.sendToJudgeTextFieldLabel
    );
    await webAction.verifyPageLabel(
      "label[for='interlocReviewState']",
      sendToJudgeData.sendToJudgeReviewStateFieldLabel
    );
  }
