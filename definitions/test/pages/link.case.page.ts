...
export class LinkCasePage {
  ...
  async verifyPageContent(caseReference: string) {
    await webAction.verifyPageLabel(
      '.govuk-heading-l',
      linkCaseTestData.linkCaseHeading
    );
    await webAction.verifyPageLabel(
      '.heading-h2',
      linkCaseTestData.linkCaseHeading2
    );
  }
  ...
  async linkCase(caseNumber: string): Promise<void> {
    ...
    await this.page
      .locator('input#linkedCase_0_0')
      .pressSequentially(caseNumber);
  ...
  async verifyCaseCannotLinkToItself() {
    let errorMessageText = (
      await this.page.locator('ul#errors li.ng-star-inserted').textContent()
    ).trim();
  ...
  async verifyCannotLinkFakeCase(caseNumber: string) {
    let errorMessageText = (
      await this.page.locator('div span.error-message').textContent()
    ).trim();
    expect(errorMessageText).toEqual(
