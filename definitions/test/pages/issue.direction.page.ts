...
export class IssueDirectionPage {
  ...
  async verifyPageContent() {
    await webActions.verifyPageLabel(
      '.govuk-caption-l',
      issueDirectionsNoticeData.eventNameCaptor
    );
    await webActions.verifyPageLabel(
      'h1.govuk-heading-l',
      issueDirectionsNoticeData.eventNameHeading
    );
    await webActions.verifyPageLabel(
      "[for='prePostHearing'] > .form-label",
      issueDirectionsNoticeData.prepostHearingLabel
    );
    await webActions.verifyPageLabel(
      "[for='directionTypeDl'] > .form-label",
      issueDirectionsNoticeData.directionTypeLabel
    );
    await webActions.verifyPageLabel(
      "[for='confidentialityType-general']",
      issueDirectionsNoticeData.sendToAllPartiesLabel
    );
    await webActions.verifyPageLabel(
      "[for='confidentialityType-confidential']",
      issueDirectionsNoticeData.selectSpecificRecipientsLabel
    );
    await webActions.verifyPageLabel(
      '#directionDueDate legend > .form-label',
    ...
    await webActions.verifyPageLabel(
      ...
      issueDirectionsNoticeData.generateNoticeLabel
    );
    await webActions.verifyPageLabel(
      "[for='generateNotice_Yes']",
  ...
  async verifySpecificRecipients() {
    await webActions.verifyPageLabel(
      '#sendDirectionNoticeToFTA legend > .form-label',
      issueDirectionsNoticeData.ftaLabel
    );
    await webActions.verifyPageLabel(
  ...
  async populateSpecificRecipients() {
    ...
    await webActions.clickElementById(
      '#sendDirectionNoticeToRepresentative_No'
    );
    // await webActions.clickElementById("#sendDirectionNoticeToOtherParty_Yes");
    await webActions.clickElementById(
      '#sendDirectionNoticeToAppellantOrAppointee_No'
    );
  }
  ...
  async enterNoticeContent(generateNoticeFlag) {
    if (!generateNoticeFlag) {
      await webActions.chooseOptionByLabel(
        '#sscsInterlocDirectionDocument_documentType',
        'Directions Notice'
      );
      ...
      await webActions.inputField(
        '#sscsInterlocDirectionDocument_documentFileName',
        'testfile1.pdf'
      );
    } else {
  ...
  async verifyErrorMsg(
    ...
    generateNoticeYesErrorMessages
  ): Promise<void> {
    ...
    if (specificRecipientsErrorMessages === true) {
      ...
      await webActions.verifyTextVisibility(
        'Appellant or Appointee is required'
      );
    }
  ...
  async populatePreHearingAppealToProceed(
    hearingOption: string,
    directionType: string,
    docTitle: string
  ): Promise<void> {
  ...
  async populatePostHearingESAAppealToProceed(
    hearingOption: string,
    directionType: string,
    docTitle: string
  ): Promise<void> {
  ...
  async populatePostHearingDLAProvideInformation(
    hearingOption: string,
    directionType: string,
    docTitle: string
  ): Promise<void> {
