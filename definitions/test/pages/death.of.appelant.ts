...
export class DeathOfAppellantPage {
  ...
  async verifyPageContent() {
    await webAction.verifyPageLabel(
      '.govuk-heading-l',
      deathOfAnAppellant.deathOfAppellantHeading
    ); //Heading Text
    await webAction.verifyPageLabel(
      "//span[.='Date of appellant death']",
      deathOfAnAppellant.dateOfAppellantDeathTextFieldLabel
    ); //Field Label
    await webAction.verifyPageLabel(
      "[for='dateOfAppellantDeath-day']",
      deathOfAnAppellant.dayAppellantDeathTextFieldLabel
    );
    await webAction.verifyPageLabel(
      "[for='dateOfAppellantDeath-month']",
      deathOfAnAppellant.monthAppellantDeathTextFieldLabel
    );
    await webAction.verifyPageLabel(
      "[for='dateOfAppellantDeath-year']",
      deathOfAnAppellant.yearAppellantDeathTextFieldLabel
    );
    await webAction.verifyPageLabel(
      "//h2[.='Appeal']",
      deathOfAnAppellant.appealSectionHeading
    );
    await webAction.verifyPageLabel(
      "//h2[.='Appellant Details']",
      deathOfAnAppellant.appellantDetailsSectionHeading
    );
  }
  ...
  async populateDeathOfAppellantPageData(yesNoOption: string) {
    ...
    await webAction.clickElementById(
      `#appeal_appellant_isAppointee_${yesNoOption}`
    );
    ...
    if (yesNoOption === 'Yes') {
      ...
      await webAction.verifyPageLabel(
        ...
        deathOfAnAppellant.appointeeDetailsSectionHeading
      );
      await webAction.verifyPageLabel(
        "//h2[.='Identity']",
        deathOfAnAppellant.identitySectionHeading
      );
      await webAction.verifyPageLabel(
        "//h2[.='Address Details']",
        deathOfAnAppellant.addressDetailsSectionHeading
      );
      await webAction.verifyPageLabel(
        "//h2[.='Contact Details']",
        deathOfAnAppellant.contactDetailsSectionHeading
      );

      await this.provideAppointeeDetails.verifyAndPopulateAppointeeDetailsPage(
        appointeeDetails
      );
    }
  }
  ...
  async verifyDeathDateNotBeIntheFutureErrorMsg(): Promise<void> {
    await webAction.verifyTextVisibility(
      'Date of appellant death must not be in the future'
    );
  }

  async verifyDeathDateNotValidErrorMsg(): Promise<void> {
    await webAction.verifyTextVisibility(
      'Date of appellant death is not valid'
    );
  }
