...
export class AllocateCtscRolePage {
  ...
  async verifyPageContent() {
    await webAction.verifyPageLabel(
      'h1 span.govuk-caption-l',
      allocateCtscRole['allocate-ctsc-role-caption']
    ); //Captor Text
    await webAction.verifyPageLabel(
      'h1.govuk-heading-l',
      allocateCtscRole['allocate-ctsc-role-heading']
    ); //Heading Text
    await webAction.verifyPageLabel(
      'label[for="allocated-ctsc-caseworker',
      allocateCtscRole['allocate-ctsc-caseworker-label']
    ); //Field Label
  }
  ...
  async allocateToUser(userEmail: string) {
    ...
    await this.page
      .locator(`//mat-option/span[contains(text(), '${userEmail}')]`)
      .click();
    await webAction.clickButton('Continue');
