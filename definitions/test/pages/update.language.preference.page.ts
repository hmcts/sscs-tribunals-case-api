...
export class UpdateLanguagePreferencePage {
  ...
  async verifyPageContent() {
    ...
    await webAction.verifyPageLabel(
      'h1.govuk-heading-l',
      languagePreferenceData.heading
    ); //Heading text
    await webAction.verifyPageLabel(
      'legend span.form-label',
      languagePreferenceData.languagePreferenceQuestionLabel
    ); //Heading text
  }

  async chooseWelshLanguage(yesOrNo: string): Promise<void> {
    await this.page
      .locator(`label[for='languagePreferenceWelsh_${yesOrNo}']`)
      .click();
  }
  ...
  async verifyCYAPageContent(): Promise<void> {
    ...
    await webAction.verifyPageLabel(
      ...
      languagePreferenceData.checkYourAnswersHeading
