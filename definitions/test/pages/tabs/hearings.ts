...
export class Hearings {
  ...
  async verifyPageContentByKeyValue(fieldLabel: string, fieldValue: string) {
    await expect(
      this.page
        .locator(
          `//*[normalize-space()="${fieldLabel}"]/../div/div[normalize-space()="${fieldValue}"]`
        )
        .first()
    ).toBeVisible();
  }
  ...
  async verifyVenueListForPaperCase(fieldLabel: string, venueTotal: number) {
    await webActions.verifyTotalElements(
      `//*[normalize-space()='${fieldLabel}']/../div/div/ul/li`,
      venueTotal
    );
  }
  ...
  async verifyExpHearingDateIsGenerated(noOfDays: string) {
    ...
    let formattedDate =
      dateUtilsComponent.formatDateToSpecifiedDateFormat(hearingBookedDate);
  ...
  async updateHearingDuration() {
    ...
    await webActions.inputField('#durationhours', '2');
