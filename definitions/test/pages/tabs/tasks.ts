...
export class Tasks {
  ...
  async clickReassignTask(taskName: string) {
    await this.page
      .locator(
        `//exui-case-task[./*[normalize-space()='${taskName}']]//a[normalize-space()='${tasksTestData.reassignTask}']`
      )
      .click();
  }
  ...
  async clickUnassignTask(taskName: string) {
    await this.page
      .locator(
        `//exui-case-task[./*[normalize-space()='${taskName}']]//a[normalize-space()='${tasksTestData.unassignTask}']`
      )
      .click();
  }

  async verifyPageContentByKeyValue(
    taskName: string,
    fieldLabel: string,
    fieldValue: string
  ) {
  ...
  async verifyNextStepNavigation(
    nextStepLink: string,
  ...
  async assignTaskToCtscUser(taskName: string, userEmail: string) {
    ...
    await expect(
      this.page.locator(
        `h1.govuk-heading-l:has-text('${tasksTestData.checkYourAnswersHeading}')`
      )
    ).toBeVisible();
    await expect(
      this.page.locator(
  ...
  async reassignTaskToTcwUser(taskName: string, userEmail: string) {
    ...
    await this.page
      .getByRole('radio', { name: tasksTestData.legalOpsRole.roleType })
      .click();
    await this.page.getByRole('button', { name: 'Continue' }).click();
    await this.page
      .locator('#inputSelectPerson')
      .fill(tasksTestData.legalOpsRole.assignToName);
    await expect(
      this.page.locator('div.mat-autocomplete-panel.mat-autocomplete-visible')
    ).toBeVisible();
    await this.page
    ...
    await expect(
      this.page.locator(
        `//td[normalize-space()='${tasksTestData.legalOpsRole.assignToName}']`
      )
    ).toBeVisible();
    await this.page
      .getByRole('button', { name: tasksTestData.reassignTaskButtonLabel })
      .click();
    ...
    await expect(
      task.getByRole('link', { name: tasksTestData.reassignTask })
    ).toBeHidden();
  }

  async cancelTask(taskName: string) {
    await this.clickCancelTask(taskName);
    await expect(this.page.locator('h1.govuk-heading-xl')).toHaveText(
      tasksTestData.cancelTaskHeading
    );
    await expect(
      this.page.locator(`exui-task-field:has-text('${taskName}')`)
    ).toBeVisible();
    await this.page
      .getByRole('button', { name: tasksTestData.cancelTaskButtonLabel })
      .click();
    await expect(
      this.page.locator(
        `//h2[normalize-space()='${tasksTestData.tasksHeading}']`
  ...
  async markTheTaskAsDone(taskName: string) {
    ...
    await expect(this.page.locator('h1.govuk-heading-xl')).toHaveText(
      tasksTestData.markAsDoneHeading
    );
    await expect(
      this.page.locator(`exui-task-field:has-text('${taskName}')`)
    ).toBeVisible();
    await this.page
      .getByRole('button', { name: tasksTestData.markAsDoneButtonLabel })
      .click();
    await expect(
      this.page.locator(
        `//h2[normalize-space()='${tasksTestData.tasksHeading}']`
  ...
  async assignTaskToAdminUser(taskName: string, userEmail: string) {
    await this.clickAssignTask(taskName);
    await this.page
      .getByRole('radio', { name: tasksTestData.adminRole.roleType })
      .click();
    await this.page.getByRole('button', { name: 'Continue' }).click();
    await this.page
      .locator('#inputSelectPerson')
      .fill(tasksTestData.adminRole.assignToName);
    await expect(
      this.page.locator('div.mat-autocomplete-panel.mat-autocomplete-visible')
    ).toBeVisible();
    await this.page
    ...
    await expect(
      this.page.locator(
        `//td[normalize-space()='${tasksTestData.adminRole.assignToName}']`
      )
    ).toBeVisible();
    await this.page
      .getByRole('button', { name: tasksTestData.assignTaskButtonLabel })
