...
export class Summary {
  ...
  async verifyPageContentByKeyValueDoesNotExist(
    fieldLabel: string,
    fieldValue: string
  ) {
    await expect(
  ...
  async verifyFieldHiddenInPageContent(fieldLabel: string) {
    await expect(
      this.page.locator(`//*[normalize-space()="${fieldLabel}"]`)
    ).toBeHidden();
  }

  async verifyPageContentLinkTextByKeyValue(
    fieldLabel: string,
    fieldValue: string
  ) {
    let text = await this.page
      .locator(`//*[normalize-space()="${fieldLabel}"]/../..//td//a`)
      .textContent();
  ...
  async verifyTitleNotPresent(fieldLabel: string) {
    await expect(
      this.page.locator(`//div/markdown/h2[contains(text(),"${fieldLabel}")]`)
    ).not.toBeVisible();
  }

  async verifyTotElemensOnPage(fieldLabel: string, fieldValue: string) {
    await webAction.verifyTotalElements(
      `//*[normalize-space()="${fieldLabel}"]/../..//td[normalize-space()="${fieldValue}"]`,
      2
    );
  }
