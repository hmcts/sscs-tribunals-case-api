...
import upload_response_payload_hmrc_053_DQ from '../../../data/payload/upload-response/upload-response-hmrc-053-DQ.json';
import upload_response_dwp_av_002_PIP from '../../../data/payload/upload-response/upload-response-dwp-av-002-PIP.json';
import action_further_evidence_payload from '../../../data/payload/action-further-evidence/action-further-evidence-other.json';

let apiContext;

async function getStartEventTokenOnCase(
  idamToken: string,
  serviceToken: string,
  userId: string,
  jurisdiction: string,
  ...
  const responseForStartAnEvent = await apiContext.get(
    `${urls.ccdApiUrl}/caseworkers/${userId}/jurisdictions/${jurisdiction}/case-types/${caseType}/cases/${caseId}/event-triggers/${eventId}/token`,
    {
      headers: {
...
async function performEventOnCaseWithEmptyBody(
  idamToken: string,
  serviceToken: string,
  userId: string,
  jurisdiction: string,
  ...
  if (responseForSubmitAnEvent.status() != 201) {
    throw new Error(
      'Error : Could not set the case to Dormant with status code - ' +
        responseForSubmitAnEvent.status() +
        ' and message ' +
        responseForSubmitAnEvent.statusText()
    );
...
async function performEventOnCaseWithUploadResponse(
  idamToken: string,
  serviceToken: string,
  userId: string,
  jurisdiction: string,
  ...
  logger.debug(
    'The value of the Upload Response Payload : ' + JSON.stringify(dataPayload)
  );
...
async function performEventOnCaseForActionFurtherEvidence(
  idamToken: string,
  serviceToken: string,
  userId: string,
  jurisdiction: string,
...
async function performEventSubmission(
  userId,
  jurisdiction,
  caseType,
  caseId,
  ...
  if (responseForSubmitAnEvent.status() != 201) {
    throw new Error(
      'Error : Could not perform the Upload Response with status code - ' +
        responseForSubmitAnEvent.status() +
        ' and message ' +
        responseForSubmitAnEvent.statusText()
    );
