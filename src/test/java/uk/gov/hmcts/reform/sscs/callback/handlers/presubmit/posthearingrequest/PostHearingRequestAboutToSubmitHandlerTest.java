package uk.gov.hmcts.reform.sscs.callback.handlers.presubmit.posthearingrequest;
...
import uk.gov.hmcts.reform.sscs.callback.handlers.presubmit.posthearingrequest.PostHearingRequestAboutToSubmitHandler;
...
class PostHearingRequestAboutToSubmitHandlerTest {
    ...
    void givenAPostHearingRequest_footerServiceIsCalledToCreateDocAndAddToBundle(
        ...
        verify(footerService).createFooterAndAddDocToCase(eq(expectedDocument.getValue().getDocumentLink()), any(),
            eq(documentType), any(), any(), eq(null), eq(null), eq(POST_HEARING_REQUEST), eq(false));
    }
