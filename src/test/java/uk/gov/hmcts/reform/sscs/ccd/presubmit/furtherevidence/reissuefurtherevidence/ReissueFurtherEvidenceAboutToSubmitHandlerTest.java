...
public class ReissueFurtherEvidenceAboutToSubmitHandlerTest {
    ...
    public void setsEvidenceHandledFlagToNoForDocumentSelected(String selectedUrl, String selectedLabel, PartyItemList newSender) {
        ...
        sscsCaseData = sscsCaseData.toBuilder()
                ...
                .reissueArtifactUi(ReissueArtifactUi.builder().resendToAppellant(YES)
                        .resendToDwp(YES)
                        .resendToRepresentative(NO)
        ...
        assertEquals(NO, selectedDocumentValue.map(AbstractDocumentDetails::getEvidenceIssued).orElse(NO));
        ...
        assertEquals(YES, otherDocumentValue.map(SscsDocumentDetails::getEvidenceIssued).orElse(NO));
    ...
    public void returnsAnErrorIfThereIsNoPartySelectedToReIssueFurtherEvidence(String includeOtherParty) {
        ...
        reissueFurtherEvidence.setResendToAppellant(NO);
        reissueFurtherEvidence.setResendToRepresentative(NO);
        reissueFurtherEvidence.setResendToDwp(NO);
        if (isYes(includeOtherParty)) {
            sscsCaseData.setOtherParties(Collections.singletonList(buildOtherParty("1")));
            sscsCaseData.getReissueArtifactUi().setOtherPartyOptions(getOtherPartyOptions(NO));
        }
    ...
    public void returnsAnErrorIfReIssuedToRepresentativeWhenThereIsNoRepOnTheAppealToReIssueFurtherEvidence() {
        sscsCaseData.getReissueArtifactUi().setResendToRepresentative(YES);
