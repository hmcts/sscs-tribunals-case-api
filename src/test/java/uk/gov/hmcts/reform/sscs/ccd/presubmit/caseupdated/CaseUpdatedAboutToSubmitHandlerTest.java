...
public class CaseUpdatedAboutToSubmitHandlerTest {
    ...
    public void givenAnAppealWithIncorrectExcludedDateStartDateAfterEndDate_thenProvideErrorMessage() {
        ...
        callback.getCaseDetails().getCaseData().getAppeal().setHearingOptions(HearingOptions.builder().wantsToAttend("Yes").build());
        callback.getCaseDetails().getCaseData().getAppeal().getHearingOptions().setExcludeDates(excludeDate);


        PreSubmitCallbackResponse<SscsCaseData> response = handler.handle(ABOUT_TO_SUBMIT, callback, USER_AUTHORISATION);
        assertThat(response.getErrors().iterator().next(), is("Start date must be before end date for excluded dates"));
    }
    ...
    public void givenAnAppealWithIncorrectExcludedDatesStartDateEmpty_thenProvideErrorMessage() {
        ...
        PreSubmitCallbackResponse<SscsCaseData> response = handler.handle(ABOUT_TO_SUBMIT, callback, USER_AUTHORISATION);
        assertThat(response.getErrors().iterator().next(), is("Add a start date for excluded dates"));
    }
    ...
    public void givenAnAppealWithIncorrectExcludedEndDate_thenProvideErrorMessage() {
        ...
        PreSubmitCallbackResponse<SscsCaseData> response = handler.handle(ABOUT_TO_SUBMIT, callback, USER_AUTHORISATION);
        assertThat(response.getErrors().iterator().next(), is("Add an end date for excluded dates"));
    }
