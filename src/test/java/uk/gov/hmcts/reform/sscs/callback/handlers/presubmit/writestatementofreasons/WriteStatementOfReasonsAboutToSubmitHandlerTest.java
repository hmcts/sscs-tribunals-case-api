package uk.gov.hmcts.reform.sscs.callback.handlers.presubmit.writestatementofreasons;
...
import uk.gov.hmcts.reform.sscs.callback.handlers.presubmit.writestatementofreasons.WriteStatementOfReasonsAboutToSubmitHandler;
...
class WriteStatementOfReasonsAboutToSubmitHandlerTest {
    ...
    void givenStatementOfReasons_footerServiceIsCalledToCreateDocAndAddToBundle() {
        ...
        verify(footerService).createFooterAndAddDocToCase(eq(expectedDocument.getValue().getDocumentLink()), any(),
            eq(STATEMENT_OF_REASONS), any(), eq(null), eq(null), eq(null), eq(null), eq(false));
    }
