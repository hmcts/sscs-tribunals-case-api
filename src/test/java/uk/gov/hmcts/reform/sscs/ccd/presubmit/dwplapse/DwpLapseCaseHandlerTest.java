...
public class DwpLapseCaseHandlerTest {
    ...
    public void givenDwpLapseCaseEventWithExistingDwpDocuments_thenSetInterlocReviewToAdminAndMoveDocsToDwpDocuments() {
        ...
        assertEquals("existing.com", response.getData().getDwpDocuments().get(0).getValue().getDocumentLink().getDocumentUrl());
        ...
        assertEquals("lt203Link", response.getData().getDwpDocuments().get(2).getValue().getDocumentLink().getDocumentUrl());
        assertEquals(DwpDocumentType.DWP_LT_203.getValue(), response.getData().getDwpDocuments().get(2).getValue().getDocumentType());
        assertEquals("awaitingAdminAction", response.getData().getInterlocReviewState());
        assertEquals("lapsed", response.getData().getDwpState());
    }
}
