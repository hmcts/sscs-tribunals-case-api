...
public class BundleAudioVideoPdfServiceTest {
    ...
    public void givenMultipleAudioVideoEvidence_thenOnlyCreateAudioVideoPdfFromMp3OrMp4() {
        ...
        assertEquals("Audio document", capture.getValue().getContent().getFirst().getDocumentType());
        assertEquals("Representative", capture.getValue().getContent().getFirst().getUploadParty());
        assertEquals(nowFormatted, capture.getValue().getContent().getFirst().getDateApproved());
        assertEquals(nowFormatted, capture.getValue().getContent().getFirst().getDateAdded());
        assertEquals("Myfilename4.mp3|gateway-link/789/binary", capture.getValue().getContent().getFirst().getDocumentUrl());
