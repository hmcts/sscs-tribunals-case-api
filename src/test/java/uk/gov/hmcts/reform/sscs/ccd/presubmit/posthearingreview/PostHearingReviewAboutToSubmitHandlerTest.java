...
class PostHearingReviewAboutToSubmitHandlerTest {
    ...
    void givenPostHearingsEnabledFalse_thenReturnFalse() {
        handler = new PostHearingReviewAboutToSubmitHandler(false, false, documentConfiguration, footerService, generateFile);
        when(callback.getEvent()).thenReturn(POST_HEARING_REVIEW);
        assertThat(handler.canHandle(ABOUT_TO_SUBMIT, callback)).isFalse();
    }
    ...
    void givenNullActionType_throwsIllegalArgumentExceptionWhenHandlingPostHearingReview(PostHearingReviewType reviewType) {
        when(callback.getCaseDetails()).thenReturn(caseDetails);
        when(caseDetails.getCaseData()).thenReturn(caseData);
