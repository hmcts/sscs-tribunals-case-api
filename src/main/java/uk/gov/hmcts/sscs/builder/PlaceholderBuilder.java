...
public class PlaceholderBuilder {
    ...
    public static JSONObject addEventPlaceHolders(Event event, JSONObject placeholdersMap, List<Hearing> hearings) {
        ...
        if(placeholdersMap != null) {
            ...
            if (APPEAL_RECEIVED.equals(eventType)) {
                ...
                placeholdersMap.put(HEARING_CONTACT_DATE_LITERAL,
                    eventDate.plus(DAYS_FROM_DWP_RESPONSE_DATE_FOR_HEARING_CONTACT, DAYS).format(ISO_INSTANT));
                ...
                ZonedDateTime decisionDateTime = BusinessDaysCalculatorService.getBusinessDay(eventDate, HEARING_DECISION_LETTER_RECEIVED_MAX_DAYS);
                placeholdersMap.put(DECISION_LETTER_RECEIVE_BY_DATE,decisionDateTime.format(ISO_INSTANT));
    ...
    private static JSONObject generateHearingJson(List<Hearing> hearings, ZonedDateTime eventDateSet) {
        JSONObject json = new JSONObject();

        if (hearings != null) {
            for (Hearing hearing : hearings) {

                ZonedDateTime localHearingDateTime = hearing.getDateTime().withZoneSameInstant(ZoneId.of("Europe/London"));
                ...
                if (formatter.format(localHearingDateTime).equals(formatter.format(eventDateSet))) {
                    ...
                    json.put(HEARING_DATETIME, formatter.format(hearing.getDateTime().withZoneSameInstant(ZoneId.of("Europe/London"))) + "T" + time.format(hearing.getDateTime()));
