...
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.config.PersonalisationMappingConstants.CLAIMANT_NAME;
...
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.domain.notify.NotificationEventType.PROCESS_AUDIO_VIDEO_WELSH;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.domain.notify.NotificationEventType.REVIEW_AND_SET_ASIDE;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.domain.notify.NotificationEventType.SET_ASIDE_GRANTED;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.domain.notify.NotificationEventType.SET_ASIDE_REFUSED;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.domain.notify.NotificationEventType.SOR_EXTEND_TIME;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.domain.notify.NotificationEventType.SOR_REFUSED;
...
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.service.LetterUtils.isAlternativeLetterFormatRequired;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.service.NotificationUtils.hasLetterTemplate;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.service.NotificationUtils.isOkToSendEmailNotification;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.service.NotificationUtils.isOkToSendSmsNotification;
import static uk.gov.hmcts.reform.sscs.notifications.gov.notify.service.NotificationValidService.isBundledLetter;
...
public class NotificationDispatchService {
    ...
    @Autowired
    public NotificationDispatchService(NotificationGateway notificationGateway, NotificationExecutionManager notificationExecutionManager,
    ...
    private boolean dispatchSmsNotification(NotificationWrapper wrapper, Notification notification, String smsTemplateId) {
        ...
        log.info("In sendSmsNotification method notificationSender is available {} ", notificationGateway != null);
    ...
    private boolean dispatchEmailNotification(NotificationWrapper wrapper, Subscription subscription, Notification notification) {
        if (isOkToSendEmailNotification(wrapper, subscription, notification, notificationValidService)) {
            ...
            log.info("In sendEmailNotification method notificationSender is available {} ", notificationGateway != null);

            notificationLog(notification, "email", notification.getEmail(), wrapper);

            return notificationExecutionManager.executeNotification(wrapper, notification.getEmailTemplate(), "Email", sendAction);
        }
    ...
    protected void dispatchLetterNotificationToAddress(NotificationWrapper wrapper, Notification notification, final Address address, SubscriptionWithType subscriptionWithType) throws NotificationClientException {
        if (address != null) {
            ...
            placeholders.putAll(getAddressPlaceholders(address, false, GOV_NOTIFY));
            ...
            log.info("In sendLetterNotificationToAddress method notificationSender is available {} ", notificationGateway != null);
            notificationLog(notification, "GovNotify letter", address.getPostcode(), wrapper);

            notificationGateway.sendLetter(
    ...
    private boolean dispatchBundledAndDocmosisLetterNotification(NotificationWrapper wrapper,
        ...
        try {
            ...
            if (isNotBlank(notification.getDocmosisLetterTemplate())) {
                ...
                NotificationExecutionManager.SendAction sendAction = alternativeLetterFormat
                    ? () -> notificationGateway.saveLettersToReasonableAdjustment(bundledLetter,
                    wrapper.getNotificationType(),
                    nameToUse,
                    wrapper.getCaseId(),
                    subscriptionWithType.getSubscriptionType())
                    : () -> notificationGateway.sendBundledLetter(wrapper,bundledLetter, nameToUse);
                ...
                if (ArrayUtils.isNotEmpty(bundledLetter)) {
                    notificationExecutionManager.executeNotification(wrapper, notification.getDocmosisLetterTemplate(), NOTIFICATION_TYPE_LETTER, sendAction);
                    return true;
                }
            }
