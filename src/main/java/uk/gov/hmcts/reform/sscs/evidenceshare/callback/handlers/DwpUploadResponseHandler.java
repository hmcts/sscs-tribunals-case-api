...
public class DwpUploadResponseHandler implements CallbackHandler<SscsCaseData> {
    ...
    public boolean canHandle(CallbackType callbackType, Callback<SscsCaseData> callback) {
        ...
        return callbackType.equals(CallbackType.SUBMITTED)
            ...
            && nonNull(callback.getCaseDetails().getCaseData().getAppeal())
            && nonNull(callback.getCaseDetails().getCaseData().getAppeal().getBenefitType())
    ...
    private void handleUc(Callback<SscsCaseData> callback) {
        ...
        boolean dwpFurtherInfo = equalsIgnoreCase(caseData.getDwpFurtherInfo(), "yes");

        boolean disputedDecision = false;
        if (caseData.getElementsDisputedIsDecisionDisputedByOthers() != null) {
            disputedDecision = equalsIgnoreCase(caseData.getElementsDisputedIsDecisionDisputedByOthers(), "yes");
        }
        ...
        if (isANewJointParty(callback, caseData)) {
            updateEventDetails(callback.getCaseDetails().getId(), JOINT_PARTY_ADDED,
                    "Joint party added", "A joint party was added to the appeal",
                    sscsCaseDetails -> {
                        SscsCaseData sscsCaseData = sscsCaseDetails.getData();
                        sscsCaseData.setJointParty(caseData.getJointParty());
                        log.info("Updated case v2 with dwp load response event {} for id {}", JOINT_PARTY_ADDED,
                                callback.getCaseDetails().getId());
                });
        }
    }
