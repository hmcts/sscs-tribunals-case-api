...
@ConditionalOnProperty(name = "feature.hearings-case-updateV2.enabled", havingValue = "false")
@SuppressWarnings({"PMD.ExcessiveImports", "PMD.UnusedFormalParameter", "PMD.TooManyMethods", "AvoidThrowingRawExceptionTypes"})
public class HearingsServiceV1 implements HearingsService {
    ...
    private void processHearingWrapper(HearingWrapper wrapper)
    ...
    private void updateHearing(HearingWrapper wrapper) throws UpdateCaseException, ListingException {
        ...
        log.info("Sending Update Hearing Request for Case ID {}", wrapper.getCaseData().getCcdCaseId());
