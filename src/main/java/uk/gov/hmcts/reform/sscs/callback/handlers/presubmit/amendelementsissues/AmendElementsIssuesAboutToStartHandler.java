...
@Component
public class AmendElementsIssuesAboutToStartHandler implements PreSubmitCallbackHandler<SscsCaseData> {
    ...
    public PreSubmitCallbackResponse<SscsCaseData> handle(CallbackType callbackType, Callback<SscsCaseData> callback, String userAuthorisation) {
        ...
        if (sscsCaseData.getAppeal().getBenefitType() != null && !sscsCaseData.getAppeal().getBenefitType().getCode().equalsIgnoreCase("uc")) {
            preSubmitCallbackResponse.addError("Error: This action is only available for UC cases.");
        }
