...
public class CaseUpdatedAboutToSubmitHandler extends ResponseEventsAboutToSubmit implements PreSubmitCallbackHandler<SscsCaseData> {
    ...
    CaseUpdatedAboutToSubmitHandler(RegionalProcessingCenterService regionalProcessingCenterService,
                                    ...
                                    @Value("${feature.case-access-management.enabled}")  boolean caseAccessManagementFeature, ReferenceDataServiceHolder refData) {
    ...
    public PreSubmitCallbackResponse<SscsCaseData> handle(CallbackType callbackType, Callback<SscsCaseData> callback, String userAuthorisation) {
        ...
        if (isHearingDurationEnabled) {
            ...
            if (!isNull(sscsCaseData.getSchedulingAndListingFields().getDefaultListingValues()) && isNull(overrideFields.getDuration())) {
                calculateDefaultDuration(sscsCaseData, caseDetailsBefore, overrideFields);
            }
        }
        return preSubmitCallbackResponse;
    }

    private void calculateDefaultDuration(SscsCaseData sscsCaseData, Optional<CaseDetails<SscsCaseData>> caseDetailsBefore, OverrideFields overrideFields) {
