...
public class Scenario7Content extends EsaTemplateContent {
    ...
    public Scenario7Content(WriteFinalDecisionTemplateBody writeFinalDecisionTemplateBody) {
        ...
        addComponent(new Paragraph(EsaTemplateComponentId.DOES_NOT_HAVE_LIMITED_CAPABILITY_FOR_WORK_PARAGRAPH.name(), getDoesHaveLimitedCapabilityForWorkSentence(writeFinalDecisionTemplateBody.getAppellantName())));
        addComponent(new Paragraph(EsaTemplateComponentId.INSUFFICIENT_POINTS_PARAGRAPH_REGULATION_29_APPLIED.name(), getInsufficientPointsSentenceRegulation29Applied(writeFinalDecisionTemplateBody.getEsaNumberOfPoints(), writeFinalDecisionTemplateBody.getRegulation29Applicable())));
        ...
        addComponent(new Paragraph(EsaTemplateComponentId.DISEASE_OR_DISABLEMENT_PARAGRAPH.name(), getRegulation29DiseaseOrDisablementSentence(writeFinalDecisionTemplateBody.getAppellantName())));
        ...
        if (!writeFinalDecisionTemplateBody.isWcaAppeal() && isNotBlank(writeFinalDecisionTemplateBody.getDwpReassessTheAward())) {
            addComponent(new Paragraph(EsaTemplateComponentId.RECOMMENDATION.name(), getRecommendationSentence(writeFinalDecisionTemplateBody.getDwpReassessTheAward())));
        }
