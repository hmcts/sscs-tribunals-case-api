...
@Component
public class ValidAppealCreatedAboutToSubmit implements PreSubmitCallbackHandler<SscsCaseData> {
    @Override
    public boolean canHandle(CallbackType callbackType, Callback<SscsCaseData> callback) {
        requireNonNull(callback, "callback must not be null");
        requireNonNull(callbackType, "callbacktype must not be null");

        return callbackType.equals(CallbackType.ABOUT_TO_SUBMIT) && callback.getEvent() == EventType.VALID_APPEAL_CREATED;
    }
    ...
    public PreSubmitCallbackResponse<SscsCaseData> handle(CallbackType callbackType, Callback<SscsCaseData> callback,
        ...
        if (!isBenefitTypeChildSupportOrUc(sscsCaseData)
            && sscsCaseData.getAppeal().getAppellant().getConfidentialityRequiredAnswer() == YesNoUnknown.UNKNOWN) {
