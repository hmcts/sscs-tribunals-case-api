...
public class Scenario15Content extends UcTemplateContent {

    public Scenario15Content(WriteFinalDecisionTemplateBody writeFinalDecisionTemplateBody) {
        ...
        addComponent(new Paragraph(WriteFinalDecisionComponentId.CONFIRMED_OR_SET_ASIDE_PARAGRAPH.name(),
                getConfirmedOrSetAsideSentence(writeFinalDecisionTemplateBody.isSetAside(), writeFinalDecisionTemplateBody.getDateOfDecision(), writeFinalDecisionTemplateBody.isHmrc(),
                        writeFinalDecisionTemplateBody.isIbca())));
        addComponent(new Paragraph(UcTemplateComponentId.DOES_HAVE_LIMITED_CAPABILITY_FOR_WORK_PARAGRAPH.name(),
                getDoesHaveLimitedCapabilityForWorkSentence(writeFinalDecisionTemplateBody.getAppellantName(), false, true, false, false,
                        writeFinalDecisionTemplateBody.getUcCapabilityAssessmentStartDate())));
        addComponent(new Paragraph(UcTemplateComponentId.INSUFFICIENT_POINTS_PARAGRAPH.name(),
                getSchedule6PointsSentence(writeFinalDecisionTemplateBody.getUcNumberOfPoints(), true, writeFinalDecisionTemplateBody.getUcSchedule6Descriptors())));
        ...
        addComponent(new Paragraph(UcTemplateComponentId.SEVERE_CRITERIA_PASSED.name(), getSevereCriteriaPassedSentence()));
