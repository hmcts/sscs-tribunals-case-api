...
public class UpdateOtherPartyMidEventHandler implements PreSubmitCallbackHandler<SscsCaseData> {
    ...
    public boolean canHandle(CallbackType callbackType, Callback<SscsCaseData> callback) {
        ...
        return callbackType.equals(CallbackType.MID_EVENT) && callback.getEvent() == UPDATE_OTHER_PARTY_DATA && nonNull(
    ...
    public PreSubmitCallbackResponse<SscsCaseData> handle(CallbackType callbackType, Callback<SscsCaseData> callback, String userAuthorisation) {
        ...
        boolean ibcCase = callback.getCaseDetails().getCaseData().isIbcCase();

        caseData.getOtherParties().forEach(party -> {
            response.addErrors(validateAddress(party.getValue().getAddress(), OTHER_PARTY, ibcCase));
            if (party.getValue().hasRepresentative()) {
                response.addErrors(
                    validateAddress(party.getValue().getRep().getAddress(), OTHER_PARTY_REPRESENTATIVE, ibcCase));

            }
            if (party.getValue().hasAppointee()) {
                response.addErrors(
                    validateAddress(party.getValue().getAppointee().getAddress(), OTHER_PARTY_APPOINTEE, ibcCase));
            }
        });
        return response;
    }

    private Set<String> validateAddress(Address address, String addressPrefix, boolean ibcCase) {
    ...
    private void validateIbcCountryAndPostcode(Address address, String addressPrefix, Set<String> validationErrors) {
    ...
    private void validateUkPostcode(Address address, String addressPrefix, Set<String> validationErrors) {
