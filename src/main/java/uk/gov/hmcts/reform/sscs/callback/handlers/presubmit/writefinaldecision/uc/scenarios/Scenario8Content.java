...
public class Scenario8Content extends UcTemplateContent {
    ...
    public Scenario8Content(WriteFinalDecisionTemplateBody writeFinalDecisionTemplateBody) {
        ...
        addComponent(new Paragraph(UcTemplateComponentId.DOES_HAVE_LIMITED_CAPABILITY_FOR_WORK_PARAGRAPH.name(),
            getDoesHaveLimitedCapabilityForWorkSentence(writeFinalDecisionTemplateBody.getAppellantName(), true, true, false, false,
                writeFinalDecisionTemplateBody.getUcCapabilityAssessmentStartDate())));
        addComponent(new Paragraph(UcTemplateComponentId.INSUFFICIENT_POINTS_PARAGRAPH_SCHEDULE_8_PARAGRAPH_4_APPLIED.name(), getInsufficientPointsSentenceNoSchedule8Paragraph4Sentence()));
