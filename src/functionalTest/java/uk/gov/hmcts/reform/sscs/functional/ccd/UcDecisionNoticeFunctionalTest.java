...
public class UcDecisionNoticeFunctionalTest extends BaseFunctionTest {
    ...
    public void scenario1_refused_non_support_group_less_than_15pts_sch6_sch8Para4NotApplies_shouldGeneratePdfWithExpectedText() throws IOException {
        ...
        try (PDDocument document = Loader.loadPDF(bytes)) {
            ...
            assertThat(pdfTextWithoutNewLines, containsString("4. In applying the Work Capability Assessment 12 points were scored from the activities and descriptors in Schedule 6 of the Universal Credit (UC) Regulations 2013. This is insufficient to meet the threshold for the test. Schedule 8, paragraph 4 of the UC Regulations did not apply."));
    ...
    public void scenario3_allowed_isSupportGroup_noSch7_Sch9Para4Applies() throws IOException {
        ...
        try (PDDocument document = Loader.loadPDF(bytes)) {
            ...
            assertThat(pdfTextWithoutNewLines, containsString("3. Joe Bloggs is to be treated as having limited capability for work-related activity from 18/11/2025."));
            ...
            assertThat(pdfTextWithoutNewLines, containsString("5. No activity or descriptor from Schedule 7 of the Universal Credit (UC) Regulations 2013 was satisfied but Schedule 9, paragraph 4 of the UC Regulations applied."));
    ...
    public void scenario4_allowed_isSupportGroup_selectionMadeForSch7() throws IOException {
        ...
        json = json.replaceFirst("writeFinalDecisionSevereCriteriaApply", null);
        byte[] bytes = callPreviewFinalDecision(json);
        try (PDDocument document = Loader.loadPDF(bytes)) {
            String pdfText = new PDFTextStripper().getText(document);
            String pdfTextWithoutNewLines = replaceNewLines(pdfText);
    ...
    public void scenario5_allowed_notSupportGroup_moreThan15Points_noSch7_NoSch9Para4() throws IOException {
        ...
        try (PDDocument document = Loader.loadPDF(bytes)) {
            ...
            assertThat(pdfTextWithoutNewLines, containsString("1. The appeal is allowed."));
            assertThat(pdfTextWithoutNewLines, containsString("2. The decision made by the Secretary of State on 17/11/2020 is set aside."));
            assertThat(pdfTextWithoutNewLines, containsString("3. Joe Bloggs has limited capability for work from 18/11/2025."));
    ...
    public void scenario8_allowed_notSupportGroup_LessThan15PointsSch6_Sch8Para4Applies_Sch9Para4Applies() throws IOException {
        ...
        try (PDDocument document = Loader.loadPDF(bytes)) {
            ...
            assertThat(pdfTextWithoutNewLines, containsString("1. The appeal is allowed."));
            assertThat(pdfTextWithoutNewLines, containsString("2. The decision made by the Secretary of State on 17/11/2020 is set aside."));
            assertThat(pdfTextWithoutNewLines, containsString("3. Joe Bloggs is to be treated as having limited capability for work and for work-related activity from 18/11/2025."));
            assertThat(pdfTextWithoutNewLines, containsString("4. This is because insufficient points were scored under Schedule 6 of the Universal Credit (UC) Regulations 2013 to meet the threshold for the Work Capability Assessment and none of the Schedule 7 activities or descriptors were satisfied."));
