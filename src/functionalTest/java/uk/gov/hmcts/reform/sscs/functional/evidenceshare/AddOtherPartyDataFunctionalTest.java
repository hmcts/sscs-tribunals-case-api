...
class AddOtherPartyDataFunctionalTest extends AbstractFunctionalTest {
    ...
    private static final String FORENAME = "Bella";
    private static final String SURNAME = "Kiki";
    ...
    void shouldTransitionToCorrectStateWhenOtherPartyDataAddedToCase() {
        ...
        await().atMost(30, SECONDS).untilAsserted(() -> {
            ...
            assertThat(cdAfterEvent.getData().getExtendedSscsCaseData().getAwareOfAnyAdditionalOtherParties()).isEqualTo(
                YesNo.YES);
            assertThat(cdAfterEvent.getData().getOtherParties().getFirst().getValue().getName().getTitle()).isEqualTo(TITLE);
            assertThat(cdAfterEvent.getData().getOtherParties().getFirst().getValue().getName().getFirstName()).isEqualTo(
                FORENAME);
            assertThat(cdAfterEvent.getData().getOtherParties().getFirst().getValue().getName().getLastName()).isEqualTo(
                SURNAME);
    ...
    private OtherParty buildOtherParty() {
        return OtherParty.builder()
            .name(Name.builder().title(TITLE).firstName(FORENAME)
                .lastName(SURNAME)
                .build())
