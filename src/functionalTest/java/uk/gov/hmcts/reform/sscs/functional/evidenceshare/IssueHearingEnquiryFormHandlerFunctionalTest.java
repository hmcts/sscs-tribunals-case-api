...
class IssueHearingEnquiryFormHandlerFunctionalTest extends AbstractFunctionalTest {
    ...
    private void issueHearingEnquiryForm(SscsCaseDetails caseDetails) {
        final String firstPartyId = caseDetails.getData().getOtherParties().stream().map(CcdValue::getValue)
            .filter(otherParty -> otherParty.getName().getFullNameNoTitle().equals("Johnny Cash")).map(OtherParty::getId)
