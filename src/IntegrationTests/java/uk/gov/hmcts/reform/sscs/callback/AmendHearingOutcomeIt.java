...
public class AmendHearingOutcomeIt extends AbstractEventIt {
    ...
    @Test
    public void noHearingOutcomesOnCaseShouldReturnError() throws Exception {
        setup("callback/amendHearingOutcomeCallback.json");
        ...
        assertThat(result.getErrors()).contains("There are no hearing outcomes on the case.");
    }

    @Test
    public void hearingOutcomesOnCaseShouldNotReturnError() throws Exception {
        setup("callback/amendHearingOutcomeCallbackWithHearingOutcome.json");
        hmcServer.stubFor(WireMock.get(
                        urlEqualTo(PATH_HEARING + "/" + PATH_CASE_ID + QUERY_PARAM))
                .willReturn(aResponse()
                        .withStatus(200)
